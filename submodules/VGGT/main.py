...
def process_frame_vggt(frame, scene_in, intrinsic_gts, prev_scale, model, depth_time=0, scale_time=0, scene_out=None, H_target=294, W_target=518):
    ...
    with get_autocast_context():
        ...
        cam2world_gts = [   
                        ...
                        f"{scene_in}/1/{frame:0>2d}.txt", 
                        f"{scene_in}/2/{frame:0>2d}.txt",
                        f"{scene_in}/3/{frame:0>2d}.txt",
                        f"{scene_in}/4/{frame:0>2d}.txt",
                        f"{scene_in}/5/{frame:0>2d}.txt",
                    ]
        ...
        image_names = [ f"{scene_in}/0/{frame:0>2d}.jpg", 
                        f"{scene_in}/1/{frame:0>2d}.jpg", 
                        f"{scene_in}/2/{frame:0>2d}.jpg",
                        f"{scene_in}/3/{frame:0>2d}.jpg",
                        f"{scene_in}/4/{frame:0>2d}.jpg",
                        f"{scene_in}/5/{frame:0>2d}.jpg",]  
        ...
        depth_vggt_cuda, _ = model.depth_head(aggregated_tokens_list, images_this, ps_idx) # (B, S, H, W, 1) (B, S, H, W)
    ...
    visualize_tracks_on_images(f'{frame:02d}', images, track_list[-1], (conf_score>0.2) & (vis_score>0.2), out_dir=os.path.join(scene_out, "vggt_track"))
...
def resolve_io_paths():
    data_root = os.path.abspath(os.path.expanduser(sys.argv[1]))
