...
def read_image(path, flags=cv2.IMREAD_COLOR, description="image"):
...
def load_nuscenes(
    ...
    if source == 'lidar':
        ...
        point_cloud = {
            'point_xyz': torch.tensor(xyz_world.astype(np.float32)),
            'point_rgb': torch.tensor((point_rgb/255).astype(np.float32)),
            'point_sem': torch.tensor(point_sem.astype(np.float32)),
            'point_instance_id': torch.tensor(point_instance_id.astype(np.float32)),
            'intrinsic3x3_ls': intrinsic3x3_ls,
            'world2cam_ls': world2cam_ls,
        }
        train_cameras = [loadCam(id, c, 1, False, False) for id, c in enumerate(train_cam_infos)] 
        ...
        for cam in range(6):
            ...
            if cam == 3: # back camera
                ego_mask = read_image(f"scene/ego_mask_{cam}.png", cv2.IMREAD_GRAYSCALE, description="ego mask")
        ...
        point_rgb = torch.from_numpy(point_rgb).float()
        point_sem = torch.from_numpy(point_sem).float()
        ...
        train_cameras = [loadCam(id, c, 2, False, False) for id, c in enumerate(train_cam_infos)]
