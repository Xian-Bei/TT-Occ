...
feature_root="${PROJECT_ROOT}/precomputed"
...
EVAL_OCC=${EVAL_OCC:-1}
PRECOMPUTE_USE_DISK=${PRECOMPUTE_USE_DISK:-0}
PRECOMPUTE_WRITE_DISK=${PRECOMPUTE_WRITE_DISK:-0}
...
SEMANTIC_MODEL_TAG=$(model_tag "$SEMANTIC_PREFIX")
DEPTH_MODEL_TAG=$(model_tag "$DEPTH_PREFIX")
FLOW_MODEL_TAG=$(model_tag "$DYNAMIC_MASK_PREFIX")
...
do
    ...
    do
        output_dir=results-$setting
        for scene in "${scenes[@]}"
        do
            data_path=$data_root/$scene
            K=10
            run_name=$output_dir/$variant_dir/$scene
            rm -rf $run_name
            mkdir -p $run_name
            extra_args=()
            if [ "$EVAL_OCC" = "1" ]; then
                extra_args+=(--eval_occ --occ3d_path "$occ3d_path" --nucraft_path "$nucraft_path")
            fi
            if [ "$USE_FUSION" = "1" ]; then
                extra_args+=(--use_fusion)
            fi
            if [ "$VERBOSE" = "1" ]; then
            ...
            python train.py -m "$run_name" -s "$data_path" \
                ...
                --occ_setting "$setting" --K "$K" --variant "$variant_arg" \
                --enable_testtime_precompute --precompute_prefetch \
                --semantic_prefix "$SEMANTIC_PREFIX" \
                --depth_prefix "$DEPTH_PREFIX" \
                --dynamic_mask_prefix "$DYNAMIC_MASK_PREFIX" \
                --precompute_use_disk "$PRECOMPUTE_USE_DISK" \
                --precompute_write_disk "$PRECOMPUTE_WRITE_DISK" \
                "${extra_args[@]}"
        done
    done
done
